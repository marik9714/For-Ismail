        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            switch (tabControl1.SelectedIndex)
            {
                ...
                case 4:
                    ...
                    break;
                case 5:
                    Query = "SELECT Поступления.Код, Поступления.Дата, Товары.Наименование, Поступления.[Количество], Поступления.Цена FROM Товары INNER JOIN Поступления ON Товары.Товар_ID = Поступления.Товар_ID;";
                    break;
                default:
       ...
       // }
        ...
        private void findprovider()
        {
            ...
            else
            {
                ...
                DataSet ds = new DataSet();
                ds.Clear();
                SqlConnectionHelper.ExecuteButtonHandle(Query, "Поставщик товара", ds, connect);
        ...
        private void button15_Click(object sender, EventArgs e)
        {
            ...
            string Query, con;
            con = @"Provider=Microsoft.Jet.OLEDB.4.0;Data Source=|DataDirectory|\Магазин2.mdb;Persist Security Info=False;";
            Query = "SELECT TOP 10 Продажи.Код, Товары.Наименование, Продажи.Количество, Товары.Цена, [Продажи].[Количество]*[Товары].[Цена] AS Сумма, [Клиенты].[Фамилия]+' '+[Клиенты].[Имя]+' '+[Клиенты].[Отчество] AS Клиент, [Сотрудники].[Фамилия]+' '+[Сотрудники].[Имя]+' '+[Сотрудники].[Отчество] AS Сотрудник, Продажи.[Номер_продажи], Продажи.[Дата_продажи] FROM Товары INNER JOIN (Сотрудники INNER JOIN(Клиенты INNER JOIN Продажи ON Клиенты.Код = Продажи.Клиент_ID) ON Сотрудники.Сотрудник_ID = Продажи.Сотрудник_ID) ON Товары.Товар_ID = Продажи.Товар ORDER BY Продажи.Количество DESC;";
            var connect = SqlConnectionHelper.GetSqlConnection();
            try
            {
                connect.Open();
